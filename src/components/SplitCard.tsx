...
export function SplitCard({
  ...
  const multiPhoto = Boolean(item.images && item.images.length > 0);
    ...
    <article className="overflow-hidden rounded-[1.25rem] bg-white shadow-[0_8px_28px_rgba(20,24,20,0.06)] sm:rounded-[1.75rem]">
      <div
        className={`grid grid-cols-1 items-stretch ${
          multiPhoto
            ? "lg:grid-cols-[minmax(0,1.7fr)_minmax(0,0.9fr)]"
            : "lg:grid-cols-2"
        } ${reverse ? "lg:[&>*:first-child]:order-2" : ""}`}
      >
        <div
          className={`order-2 flex flex-col justify-between p-5 sm:p-8 md:p-10 lg:order-none ${
            multiPhoto ? "lg:p-7 xl:p-8" : ""
          }`}
        >
          <div>
            {item.eyebrow ? (
            ...
            <p
              className={`mt-3 text-pretty text-[0.92rem] leading-relaxed text-[var(--ink-soft)] sm:text-[0.95rem] ${
                multiPhoto ? "max-w-sm" : "max-w-md"
              }`}
            >
              {item.blurb}
            </p>
            ...
            <div className="mt-7 sm:mt-10">
              {item.priceLabel ? (
                <p className="text-[1.02rem] font-semibold tracking-tight text-[var(--ink)] sm:text-xl">
                  <PriceText value={item.priceLabel} />{" "}
        ...
        <div
          className={`relative order-1 w-full overflow-hidden bg-[var(--bg-soft)] lg:order-none ${
            multiPhoto ? "lg:min-h-[420px] xl:min-h-[480px]" : "lg:min-h-[460px]"
          }`}
        >
          {item.mapSrc ? (
            ...
            <div className="grid h-full grid-cols-2 gap-1.5 p-1.5 sm:gap-2 sm:p-2 lg:absolute lg:inset-0">
              <div className="relative min-h-[220px] overflow-hidden bg-[var(--bg-soft)] sm:min-h-[280px] lg:min-h-0">
                <Image
                  src={item.image}
                  alt={item.title}
                  fill
